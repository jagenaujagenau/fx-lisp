;;;; prompt.lisp — system prompt, ported verbatim from builtins/context.zig.

(in-package #:fx.prompt)

(defparameter +identity-section+
...
(defparameter +workspace-section+
  "# Workspace behavior
...
(defparameter +source-routing-section+
  "# Source routing
...
(defparameter +interaction-section+
  "# Interaction
...
(defparameter +tools-and-verification-section+
...
(defun gateway-system-prompt ()
  (concatenate 'string
               +identity-section+ (string #\Newline)
               +workspace-section+ (string #\Newline)
               +source-routing-section+ (string #\Newline)
               +interaction-section+ (string #\Newline)
               +safety-section+ (string #\Newline)
               +tools-and-verification-section+))

(defun %command-output (command)
  "Run COMMAND (list of strings), return trimmed stdout or NIL."
  (handler-case
...
(defun runtime-context ()
  "Transient runtime context appended to the system prompt each turn."
  (let* ((cwd (namestring *default-pathname-defaults*))
         (branch (%command-output '("git" "rev-parse" "--abbrev-ref" "HEAD")))
         (dirty (when branch (%command-output '("git" "status" "--porcelain")))))
    (format nil "# Runtime context~%~%- cwd: ~a~%- os: ~a~%- shell: ~a~%- date: ~a~a"
            cwd
            (or (fx.util:getenv "OSTYPE") (software-type) "unknown")
            (or (fx.util:getenv "SHELL") "unknown")
            (fx.util:iso8601-now)
            (if branch
                (format nil "~%- git branch: ~a (~a)"
                        branch (if dirty "dirty" "clean"))
                ""))))
